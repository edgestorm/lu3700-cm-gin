...
void display_table(struct display_table *table, unsigned int count)
{
    ...
    for(i = 0; i < count; i++) {
        ...
        switch (reg) {
            ...
            default:
                mddi_host_register_cmds_write8(reg, table[i].count, table[i].val_list, 1, 0, 0); //ojh temp
...
/* Block temporaly till vsync implement */
...
static void hitachi_workaround(void)
{
	...
	display_table(mddi_hitachi_position_table, sizeof(mddi_hitachi_position_table) / sizeof(struct display_table));
#elif defined(CONFIG_FB_MSM_MDDI_HITACHI_HVGA) && defined(CONFIG_MACH_MSM7X27_THUNDERA)
	display_table(mddi_hitachi_2c,
			sizeof(mddi_hitachi_2c) / sizeof(struct display_table));
#elif defined(CONFIG_FB_MSM_MDDI_HITACHI_HVGA) && (defined(CONFIG_MACH_MSM7X27_SU370) || defined(CONFIG_MACH_MSM7X27_KU3700) || defined(CONFIG_MACH_MSM7X27_LU3700))
