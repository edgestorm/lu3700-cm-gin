...
#define LCD_LED_MAX 22 /* 13.5mA */ /* TODO: it should be modified for saving currnet */
...
static struct aat28xx_ctrl_tbl aat2870bl_alc_tbl[] = {
	...
	{ 0x0E, 0x71 },  /* SNSR_LIN_LOG=linear, ALSOUT_LIN_LOG=log, RSET=4k~16k,
...
int aat28xx_ldo_set_level(struct device *dev, unsigned num, unsigned vol)
{
	...
	if (num > 0 && num <= AAT28XX_LDO_NUM) {
		if ((adap=dev_get_drvdata(dev)) && (client=i2c_get_adapdata(adap))) {
#if (!CONFIG_MACH_MSM7X27_SU310 && !CONFIG_MACH_MSM7X27_LU3100) // for compile error dr.ryu
