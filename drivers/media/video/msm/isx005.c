...
#if defined (CONFIG_MACH_MSM7X27_THUNDERG) || defined (CONFIG_MACH_MSM7X27_SU370) || defined (CONFIG_MACH_MSM7X27_KU3700) || defined (CONFIG_MACH_MSM7X27_LU3700)
...
static int32_t prevew_mode_status = CAMERA_MODE_DEFAULT;
static uint16_t cm_light_check_sts = 0;
static uint16_t iso_check_sts = CAMERA_ISO_AUTO;
static uint16_t pclk_pad_strength_value = CAMERA_PCLK_PAD_STRENGTH_6MA;
	...
	unsigned short waddr, unsigned short wdata, enum isx005_width width)
{
	...
	switch (width) {
		case BYTE_LEN:	
		   ...
		   printk(KERN_ERR "[### check] i2c_write , addr = 0x%x, val = 0x%x!\n", waddr, wdata);      
			break;
	...
	unsigned short raddr, unsigned short *rdata, enum isx005_width width)
{
   ...
   printk(KERN_ERR "[### check] i2c_read , addr = 0x%x, val = 0x%x!\n", (unsigned int)raddr, (unsigned int)*rdata);
...
static int isx005_reg_init(void)
{
    ...
    printk(KERN_ERR "###john.park ; [CHECK]%s: mode init\n, prevew_mode_status = %d\n", __func__, prevew_mode_status);
    printk(KERN_ERR "[###john.park] isx005_reg_init, mode = %d!\n", prevew_mode_status);                
    iso_check_sts = CAMERA_ISO_AUTO;

	/* Configure sensor for Initial setting (PLL, Clock, etc) */
	if(pclk_pad_strength_value == CAMERA_PCLK_PAD_STRENGTH_6MA)
	{
		printk(KERN_ERR "###john.park ; [CHECK]%s: 6MA pad_strength_value \n", __func__);
		for (i = 0; i < isx005_regs.init_reg_settings_6ma_size; ++i) {
			rc = isx005_i2c_write(isx005_client->addr,
				isx005_regs.init_reg_settings_6ma[i].register_address,
				isx005_regs.init_reg_settings_6ma[i].register_value,
				isx005_regs.init_reg_settings_6ma[i].register_length);
		}
	}
	else// (pclk_pad_strength_value == CAMERA_PCLK_PAD_STRENGTH_4MA)
	{
		printk(KERN_ERR "###john.park ; [CHECK]%s: 4MA pad_strength_value \n", __func__);	
		for (i = 0; i < isx005_regs.init_reg_settings_4ma_size; ++i) {
			rc = isx005_i2c_write(isx005_client->addr,
				isx005_regs.init_reg_settings_4ma[i].register_address,
				isx005_regs.init_reg_settings_4ma[i].register_value,
				isx005_regs.init_reg_settings_4ma[i].register_length);
		}
	}
...
static int isx005_reg_preview(void)
{
  ...
  for(i=0; i<300;i++)
  {
      ...
      if(cm_changed_status & 0x0002)
      {
		...
		if(iso_check_sts == CAMERA_ISO_AUTO)
		{
			isx005_i2c_write(isx005_client->addr,0x4018,0xCA03, WORD_LEN);// AGCLOWSCL_L : 6.3dB  
			isx005_i2c_write(isx005_client->addr,0x401C,0xCA03, WORD_LEN);// AGCHISCL_L : 10.8dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0xFF01, WORD_LEN);// AGCMAXSCL_L : DB  		
		}
		else if(iso_check_sts == CAMERA_ISO_100)
		{
			// ISO 100 (0dB -)
			isx005_i2c_write(isx005_client->addr,0x4018,0x3301, WORD_LEN);// AGCLOWSCL_L :0 dB 
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);// AGCHISCL_L : -dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);// AGCMAXSCL_L : - DB  		
		}
		else if(iso_check_sts == CAMERA_ISO_200)
		{
			// ISO 200 (4.5dB- 768)
			isx005_i2c_write(isx005_client->addr,0x4018,0x3004, WORD_LEN);// AGCLOWSCL_L : 4.5dB
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);// AGCHISCL_L : -dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);// AGCMAXSCL_L : - DB 		
		}
		else if(iso_check_sts == CAMERA_ISO_400)
		{
			// ISO 200 (10.8dB- 1843)
			isx005_i2c_write(isx005_client->addr,0x4018,0x6008, WORD_LEN);// AGCLOWSCL_L : 10.8dB
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);// AGCHISCL_L : -dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);// AGCMAXSCL_L : - DB 		
		}
		else{ 
	       	printk("[%s]:Invalid ISO value : %d\n", __func__, iso_check_sts);
		}
        break;  
      }
      else 
...
static int isx005_reg_video(void)
{
	int rc = 0;
	int i;
      unsigned short cm_changed_status = 0;
	...
	/* Configure sensor for Preview mode */
	for (i = 0; i < isx005_regs.video_reg_settings_size; ++i) {
		rc = isx005_i2c_write(isx005_client->addr,
		  isx005_regs.video_reg_settings[i].register_address,
		  isx005_regs.video_reg_settings[i].register_value,
		  isx005_regs.video_reg_settings[i].register_length);
...
static int isx005_reg_video_for_delivering(void)
{
	int rc = 0;
	int i;
      unsigned short cm_changed_status = 0;
	...
	/* Configure sensor for Preview mode */
	for (i = 0; i < isx005_regs.video_15fps_reg_settings_size; ++i) {
		rc = isx005_i2c_write(isx005_client->addr,
		  isx005_regs.video_15fps_reg_settings[i].register_address,
		  isx005_regs.video_15fps_reg_settings[i].register_value,
		  isx005_regs.video_15fps_reg_settings[i].register_length);
...
static int isx005_reg_snapshot(void)
{
	...
	for(i=0; i<300;i++)
	{
		...
		if(iso_check_sts == CAMERA_ISO_AUTO)
		{
			isx005_i2c_write(isx005_client->addr,0x4018,0xCA03, WORD_LEN);// AGCLOWSCL_L : 6.3dB  
			isx005_i2c_write(isx005_client->addr,0x401C,0xCA03, WORD_LEN);// AGCHISCL_L : 10.8dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0xCA03, WORD_LEN);// AGCMAXSCL_L : 16.5DB  
		}
		else if(iso_check_sts == CAMERA_ISO_100)
		{
			// ISO 100 (6dB - 768)
			isx005_i2c_write(isx005_client->addr,0x4018,0x2f05, WORD_LEN);// AGCLOWSCL_L : 6dB 
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);// AGCHISCL_L : -dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);// AGCMAXSCL_L : - DB  
		}
		else if(iso_check_sts == CAMERA_ISO_200)
		...
		{
			// ISO 400 (16.2dB -2764 )
			isx005_i2c_write(isx005_client->addr,0x4018,0xf60b, WORD_LEN);// AGCLOWSCL_L : 16.2dB
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);// AGCHISCL_L : -dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);// AGCMAXSCL_L : - DB
		}
		else{ 
...
static int isx005_set_sensor_mode(int mode)
{
    ...
    printk(KERN_ERR "###john.park ; [CHECK]%s: mode check, prevew_mode_status->%d , mode -> %d\n", __func__, prevew_mode_status, mode);
    if(mode == SENSOR_PREVIEW_MODE)
    {
      if (prevew_mode_status == CAMERA_MODE_DEFAULT)  // Init -> preview mode 
        { 
            prevew_mode_status = mode;
            printk(KERN_ERR "###john.park ; [CHECK]%s: change mode, prevew_mode_status -> %d\n", __func__, prevew_mode_status);
            
        }
      else  //prevew_mode_status == CAMERA_PREVIEW_MODE_FPS || prevew_mode_status == CAMERA_CAMCORD_MODE_FPS||prevew_mode_status == CAMERA_CAMCORD_FOR_DELIVERING_MODE_FPS
        {
            printk(KERN_ERR "###john.park ; [CHECK]%s: skip mode, prevew_mode_status -> %d\n", __func__, prevew_mode_status);
            return rc;
       }
    }
    else
      {
          prevew_mode_status = CAMERA_MODE_DEFAULT;
            printk(KERN_ERR "###john.park ; [CHECK]%s: snapshot mode, prevew_mode_status -> %d\n", __func__, prevew_mode_status);
...
//	CDBG("Sensor Mode : %d, rc = %d\n", mode, rc);
    printk(KERN_ERR "###john.park ; [CHECK]%s: Sensor Mode -> %d, rc -> %d\n", __func__, mode, rc);
...
static int isx005_set_default_focus(void)
{
	int32_t rc = 0;
       ...
       printk(KERN_ERR "###john.park ; [CHECK]%s ", __func__);
...
static int isx005_set_iso(int iso)
{
	...
	if(iso == iso_check_sts)
	{
		printk(KERN_ERR "###john.park ; [CHECK]%s: skip this function, iso -> %d\n", __func__, iso);
		return rc;
	}

       printk(KERN_ERR "###john.park ; [CHECK]%s: iso -> %d\n", __func__, iso);
...
//					0x01E5, 0x00, BYTE_LEN);
			isx005_i2c_write(isx005_client->addr,0x4018,0xCA03, WORD_LEN);// AGCLOWSCL_L : 6.3dB  
			isx005_i2c_write(isx005_client->addr,0x401C,0xCA03, WORD_LEN);// AGCHISCL_L : 10.8dB   
			isx005_i2c_write(isx005_client->addr,0x4020,0xFF01, WORD_LEN);// AGCMAXSCL_L : 14.4DB  		
			break;
...
static int32_t isx005_set_scene_mode(int8_t mode)
{
	...
	{
		printk(KERN_ERR "###john.park ; [CHECK]%s: skip this function, scene_mode -> %d\n", __func__, mode);
		return rc;
	}
...
static int32_t isx005_set_preview_mode(int8_t mode)
{
	int32_t rc = 0;
	int retry = 0;  

//    printk(KERN_ERR "###john.park ; [CHECK]%s: mode check, prevew_mode_status->%d , mode -> %d\n", __func__, prevew_mode_status, mode);
//    (prevew_mode_status == mode) ? return rc; : prevew_mode_status = mode;
    if(prevew_mode_status == mode)
      { 
          printk(KERN_ERR "###john.park ; [CHECK]%s: skip mode, mode -> %d\n", __func__, mode);
          return rc; 
      }
    else
      { 
         prevew_mode_status = mode;
         printk(KERN_ERR "###john.park ; [CHECK]%s: change mode, mode -> %d, prevew_mode_status -> %d\n", __func__, mode, prevew_mode_status);
      }

	switch (mode) {
		case CAMERA_PREVIEW_MODE_FPS:
          printk(KERN_ERR "###john.park ; [CHECK]%s:preview mode reg set\n", __func__);
    		for (retry = 0; retry < 3; ++retry) {
    			rc = isx005_reg_preview();
    			if (rc < 0)
    				printk(KERN_ERR "[CHECK]%s:Sensor Preview Mode Fail\n", __func__);
    			else
    				break;
    		}
			break;
		...
		case CAMERA_CAMCORD_MODE_FPS:
          printk(KERN_ERR "###john.park ; [CHECK]%s: camcord mode reg set\n", __func__);      
    		for (retry = 0; retry < 3; ++retry) {
    			rc = isx005_reg_video();
    			if (rc < 0)
    				printk(KERN_ERR "[CHECK]%s:Sensor Preview Mode Fail\n", __func__);
		...
		case CAMERA_CAMCORD_MODE_FPS_FOR_DELIVERING:
          printk(KERN_ERR "###john.park ; [CHECK]%s: camcord mode reg set\n", __func__);      
    		for (retry = 0; retry < 3; ++retry) {
    			rc = isx005_reg_video_for_delivering();
    			if (rc < 0)
    				printk(KERN_ERR "[CHECK]%s:Sensor Preview Mode Fail\n", __func__);
    			else
    				break;
		...
		default:
			printk(KERN_ERR "###john.park ; [CHECK]%s:Incorrect preview mode value\n", __func__);
	}
	return rc;

}

static int32_t isx005_set_pclk_pad_strength(int8_t value)
{
	int32_t rc=0;

       printk(KERN_ERR "###john.park ; [CHECK]%s: pad_strength value -> %d\n", __func__, value);

	pclk_pad_strength_value = value;
...
static int32_t isx005_set_brightness(int8_t brightness)
{
	int32_t rc=0;
       ...
       printk(KERN_ERR "###john.park ; [CHECK]%s: brightness -> %d\n", __func__, brightness);
...
static int dequeue_sensor_config(int cfgtype, int mode)
{
	...
	switch (cfgtype) {
		...
		case CFG_SET_PREVIEW_MODE:
			rc = isx005_set_preview_mode(mode);
			break;

		case CFG_SET_PCLK_PAD:
			rc = isx005_set_pclk_pad_strength(mode);
			break;
...
int isx005_sensor_config(void __user *argp)
{
	...
	switch (cfg_data.cfgtype) {
		...
		case CFG_SET_PREVIEW_MODE:
			rc = isx005_set_preview_mode(cfg_data.mode);
			break;

		case CFG_SET_PCLK_PAD:
			rc = isx005_set_pclk_pad_strength(cfg_data.mode);
			break;
