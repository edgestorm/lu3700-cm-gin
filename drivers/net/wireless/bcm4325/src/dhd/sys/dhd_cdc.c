...
#ifdef BDC
	struct bdc_header *h;
#if defined(CONFIG_LGE_BCM432X_PATCH) && (defined(CONFIG_LGE_WMM_KT) || defined(CONFIG_LGE_WMM_SKT))
	uint8 priority;
#endif
#ifdef APSTA_PINGTEST
...
#elif defined(CONFIG_LGE_BCM432X_PATCH) && defined(CONFIG_LGE_WMM_SKT)
	priority = (PKTPRIO(pktbuf) & BDC_PRIORITY_MASK);
	if(priority == 5){
		h->priority = 6;
	} else {
