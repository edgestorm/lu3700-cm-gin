...
/* from device spec */
// start dr.ryu 2010-09-15 LAB1_FW : 박성원 연구원 요청 사항{
#if 0
...
#if defined(CONFIG_MACH_MSM7X27_SU310) || defined(CONFIG_MACH_MSM7X27_LU3100)
	unsigned int positive_persistence;	/* kk - implement sw persistent to eliminate spike noise (above threshold) */
	unsigned int negative_persistence;	/* kk - implement sw persistent to eliminate spike noise (below threshold) */
...
#if defined(CONFIG_MACH_MSM7X27_SU310) || defined(CONFIG_MACH_MSM7X27_LU3100)
	data->detection_threshold = threshold;	/* kk for hsyteresis purpose */
#endif
...
apds9801_work_func(struct work_struct *work)
{
	...
	if (interrupt_data & IntpNegStat) {
...
#if defined(CONFIG_MACH_MSM7X27_SU310) || defined(CONFIG_MACH_MSM7X27_LU3100)
			/* kk start sw persistence check */
			data->negative_persistence++;
			data->positive_persistence=0;	/* just in case */
#else
			data->vout = PROX_INPUT_FAR;
#endif
	}
		...
		data->negative_persistence = 0;	/* just in case */
#else
		data->vout = PROX_INPUT_NEAR;
#endif
	}
		...
		if (data->positive_persistence) {
			/* kk sw persistence check with NO interrupt */

			if (adc_data > data->detection_threshold) {
				data->positive_persistence++;
				if (data->positive_persistence >= PS_SW_PERSISTENCE) {

					/* kk, switch threshold to lower value as Hysteresis */
					mutex_lock(&data->update_lock);
			...
			if (adc_data < data->detection_threshold) {
				...
				if (data->negative_persistence >= PS_SW_PERSISTENCE) {
					...
					/* kk, switch back to original threshold */
					mutex_lock(&data->update_lock);
...
static int apds9801_irq_handler(int irq, void *dev_id)
{
	...
	/* __cancel_delayed_work(&data->dwork); kk - can cancel the previous queued work if there is */
#endif
...
#if defined(CONFIG_MACH_MSM7X27_SU310) || defined(CONFIG_MACH_MSM7X27_LU3100)
			data->positive_persistence = 0;	/* kk reset to default value 0 */
			data->negative_persistence = 0;	/* kk reset to default value 0 */
#endif
...
/*
	pm_message_t dummy_state;
	if (apds9801_pdev == NULL) {
		printk(KERN_INFO "apds9801_pdev device is null\n");
		return -1;
	}

	apds9801_resume(apds9801_pdev->client);

	struct apds9801_data *pdev = i2c_get_clientdata(apds9801_pdev->client);
	*value = !(pdev->last_vout);

	apds9801_suspend(apds9801_pdev->client, dummy_state);
*/
}
