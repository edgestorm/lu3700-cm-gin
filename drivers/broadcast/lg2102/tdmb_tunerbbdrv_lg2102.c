...
extern void LGD_WRITE_EBI2_ADDRESS(void *addr);
...
int8 tunerbb_drv_lg2102_init(void)
{
	...
	m_unIntCtrl 		= (LGD_INTERRUPT_POLARITY_HIGH| \
						   LGD_INTERRUPT_PULSE | \
						   LGD_INTERRUPT_AUTOCLEAR_ENABLE | \
						   (LGD_INTERRUPT_PULSE_COUNT & LGD_INTERRUPT_PULSE_COUNT_MASK));
	
	printk("tunerbb_drv_lg2102_init() m_ucCommandMode %d\n", m_ucCommandMode);
...
boolean tunerbb_drv_lg2102_set_ebi2_address(void *addr)
{
	LGD_WRITE_EBI2_ADDRESS(addr);
