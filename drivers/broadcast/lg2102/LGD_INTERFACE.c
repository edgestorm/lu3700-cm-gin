...
//ojh UPLOAD_MODE_INFO	m_ucUploadMode 		= STREAM_UPLOAD_SPI;
...
//ojh	tdmb_lg2102_spi_write_read(auiTxBuff, 4, auiRxBuff, 2);
...
//ojh	tdmb_lg2102_spi_write_read(auiTxBuff, 6, auiRxBuff, 0);
	LGD_INTERRUPT_FREE();
	return LGD_SUCCESS;
}
...
LGD_UINT8 LGD_SPI_READ_BURST(LGD_UINT8 ucI2CID, LGD_UINT16 uiAddr, LGD_UINT8* pBuff, LGD_UINT16 wSize)
{
	...
	for(uiLoop = 0; uiLoop < nIndex; uiLoop++){
		...
		//ojh tdmb_lg2102_spi_write_read(auiBuff, 4, m_auiRxBuff, anLength[uiLoop]);
