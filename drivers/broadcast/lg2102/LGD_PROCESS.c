...
extern void LGD_EBI_WRITE_ADDRESS_SETUP(LGD_UINT8 *address);
extern void LGD_MUST_DELAY( LGD_UINT16 msDelay);
...
LGD_UINT8 LGD_CHIP_STATUS(LGD_UINT8 ucI2CID)
{
	...
	if((uiChipID & 0xF00) < 0xA00)
	{
		printk("LGD_CHIP_STATUS Error uiChiID = (0x%X)\n", uiChipID);
		return LGD_ERROR;
	}
	return LGD_SUCCESS;
}
...
LGD_UINT8 LGD_INIT(LGD_UINT8 ucI2CID)
{
	...
	{
		printk("LGD_INIT Mode ERR mode =(%d)\n", m_ucTransMode);
		return LGD_ERROR;
	}

	pInfo->ucTransMode = m_ucTransMode;
	if(LGD_PLL_SET(ucI2CID) != LGD_SUCCESS)
	{
		printk("LGD_INIT PLL_SET ERR\n");
		return LGD_ERROR;
	}
	if(LGD_CHIP_STATUS(ucI2CID) != LGD_SUCCESS)
	{
		printk("LGD_INIT CHIP STATUS ERR\n");
		return LGD_ERROR;
	}
	...
	while(1)
	{
		...
		if(!wIsNullSync){
			...
			if(wData < 0x0200){
				...
				printk("[LG2102] ERROR_SYNC_NO_SIGNAL\n");
				break;
			}
		}
		if(wOperState >= 0x5){
...
void LGD_WRITE_EBI2_ADDRESS(void *addr)
{
	return  LGD_EBI_WRITE_ADDRESS_SETUP( (LGD_UINT8 *)addr);
