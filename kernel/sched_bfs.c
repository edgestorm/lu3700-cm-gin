...
#define RESCHED_US	(100) /* Reschedule if less than this many I卹s left */
			  ...
			  int wake_flags)
{
	...
	/* This barrier is undocumented, probably for p->state? a??a?? */
	smp_wmb();
	...
	/* state is a volatile long, a?使a?Ｅa??a?|aＸ?a?Ｅa??a?７a?見a?? */
...
static void task_running_tick(struct rq *rq)
{
	 ...
	 * less than RESCHED_US I卹s of time slice left they will be rescheduled.
	 */
	if (rq->dither) {
...
{
	/* NOT a real fix but will make voluntary preempt work. e|／e易０a?見a足? */
