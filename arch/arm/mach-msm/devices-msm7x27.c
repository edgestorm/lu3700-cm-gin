...
static int smpl_reset_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int len;
	char *p = page;
	int *l_testmode = (int *)data;
...
static int testmode_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int len;
	char *p = page;
...
static int sw_version_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int len;
	char *p = page;
...
void create_sw_version_status(void)
{
	static unsigned char *p_smem_sw_ver_status ;
	int size ;
	
	p_smem_sw_ver_status = smem_get_entry( SMEM_ID_VENDOR1 ,&size ) ;

	create_proc_read_entry("sw_version", S_IRUGO, NULL , sw_version_read_proc, p_smem_sw_ver_status);
}

static int qpst_enable_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int len;
	char *p = page;
	short *qpst_enable = (short *)data;

	p += sprintf(p, "qpst_enable=%d\n", *qpst_enable);
...
struct clk_lookup msm_clocks_7x27[] = {
	...
	CLK_VOTER("ebi1_tdmb_clk",	EBI_TDMB_CLK,	"ebi1_clk", NULL, 0),
