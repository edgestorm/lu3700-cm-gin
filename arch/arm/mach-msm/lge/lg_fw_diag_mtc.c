...
//  byte *bmp_data;
  ssize_t bmp_size;
  unsigned int left, top, width, height, bytes_pixel, i;
  unsigned char *framebuffer, *pSrc, *pDst;
  
  left = pReq->mtc_req.capture.left;
  top = pReq->mtc_req.capture.top;
  width = pReq->mtc_req.capture.width;
  height = pReq->mtc_req.capture.height;
  bmp_size = width*height*2;
  printk(KERN_INFO "[MTC]mtc_capture_screen : %d, %d, %d, %d\n", left, top, width, height);
  
  if(left+width>lcd_info.width_max || top+height>lcd_info.height_max)
  {
    printk(KERN_ERR "[MTC] invalid capture size\n");
    return NULL;
  }
  rsp_len = sizeof(mtc_capture_rsp_type) - MTC_SCRN_BUF_SIZE_MAX + bmp_size;
...
//  memset(&fb_varinfo, 0, sizeof(struct fb_var_screeninfo));
  ...
  pRsp->mtc_rsp.capture.bmp_width = width;
  pRsp->mtc_rsp.capture.bmp_height = height;
  ...
  if(width == lcd_info.width_max && height == lcd_info.height_max)
  {
//  bmp_size = read_framebuffer( bmp_data, &fb_varinfo, req);
  ...
  {
    framebuffer = kmalloc(MTC_SCRN_BUF_SIZE_MAX, GFP_KERNEL);
    if (framebuffer == NULL) {
      printk(KERN_ERR "[MTC] kmalloc failed\n");
      diagpkt_free(pRsp);
      return NULL;
    }
    ...
    //copy partial image
    memset(pRsp->mtc_rsp.capture.bmp_data, 0, bmp_size);
    bytes_pixel = lcd_info.bits_pixel / 8;
    pSrc = framebuffer + (top*lcd_info.width_max + left) * bytes_pixel;
    pDst = pRsp->mtc_rsp.capture.bmp_data;

    for(i=0; i<height; i++)
    {
      memcpy(pDst, pSrc, width * bytes_pixel);
      pDst += width * bytes_pixel;
      pSrc += lcd_info.width_max * bytes_pixel;
    }
