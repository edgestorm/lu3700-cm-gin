...
int lu3700_pwrsink_suspend_noirq(struct device *dev)
{
...
int lu3700_pwrsink_resume_noirq(struct device *dev)
{
...
static struct dev_pm_ops lu3700_pwrsink_data = {
	.suspend_noirq = lu3700_pwrsink_suspend_noirq,
	.resume_noirq = lu3700_pwrsink_resume_noirq,
};

static struct platform_device lu3700_pwrsink_device = {
	.name = "lge-pwrsink",
	.id = -1,
	.dev = {
		.platform_data = &lu3700_pwrsink_data,
	},
};
...
{
	lcd_bl_power_state = BL_POWER_RESUME;
	platform_device_register(&lu3700_pwrsink_device);
}
