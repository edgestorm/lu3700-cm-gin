...
#include "board-ku3700.h"

static u32 ku3700_battery_capacity(u32 current_soc)
{
	if(current_soc > 100)
		current_soc = 100;
...
static struct msm_psy_batt_pdata msm_psy_batt_data = {
	...
	.calculate_capacity		= ku3700_battery_capacity,
};
...
/* KU3700 Board Vibrator Functions for Android Vibrator Driver */
...
static void ku3700_desk_dock_detect_callback(int state)
{
	int ret;
...
static int ku3700_register_callback(void)
{
	rpc_server_hs_register_callback(ku3700_desk_dock_detect_callback);

	return 0;
}

static int ku3700_gpio_carkit_work_func(void)
{
	return DOCK_STATE_UNDOCKED;
}

static char *ku3700_gpio_carkit_print_state(int state)
{
	return dock_state_string[state];
}

static int ku3700_gpio_carkit_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned ku3700_carkit_gpios[] = {
};

static struct lge_gpio_switch_platform_data ku3700_carkit_data = {
	.name = "dock",
	.gpios = ku3700_carkit_gpios,
	.num_gpios = ARRAY_SIZE(ku3700_carkit_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = ku3700_gpio_carkit_work_func,
	.print_state = ku3700_gpio_carkit_print_state,
	.sysfs_store = ku3700_gpio_carkit_sysfs_store,
	.additional_init = ku3700_register_callback,
};

static struct platform_device ku3700_carkit_device = {
	.name = "lge-switch-gpio",
	.id = 0,
	.dev = {
		.platform_data = &ku3700_carkit_data,
	},
};
...
int ku3700_vibrator_power_set(int enable)
{
	static int is_enabled = 0;
	struct device *dev = ku3700_backlight_dev();
...
int ku3700_vibrator_pwn_set(int enable, int amp)
{
...
int ku3700_vibrator_ic_enable_set(int enable)
{
	/* nothing to do, ku3700 does not using Motor Enable pin */
	return 0;
}
int ku3700_vibrator_gpio_request(void)
{
	int rc = 0;
...
static struct android_vibrator_platform_data ku3700_vibrator_data = {
	.enable_status = 0,	
	.power_set = ku3700_vibrator_power_set,
	.pwm_set = ku3700_vibrator_pwn_set,
	.ic_enable_set = ku3700_vibrator_ic_enable_set,
	.gpio_request = ku3700_vibrator_gpio_request,
	.amp_value = 92,
};
...
static struct platform_device android_vibrator_device = {
	...
	.id = -1,
	.dev = {
		.platform_data = &ku3700_vibrator_data,
	},
};
/* misc platform devices */
static struct platform_device *ku3700_misc_devices[] __initdata = {
	&msm_batt_device,
	&android_vibrator_device,
	&ku3700_carkit_device,
};

/* main interface */
void __init lge_add_misc_devices(void)
{
	platform_add_devices(ku3700_misc_devices, ARRAY_SIZE(ku3700_misc_devices));
